...
void m19::postfix_writer::do_function_definition_node(m19::function_definition_node *const node, int lvl) {
    ...
    if(node->type()->name() != basic_type::TYPE_VOID){
        ...
        if(returnSymbol->type()->name() == basic_type::TYPE_DOUBLE){
            _pf.LDDOUBLE();
            _pf.STFVAL64();
        }
    }
...
void m19::postfix_writer::do_function_call_node(m19::function_call_node *const node, int lvl) {
    ...
    if (node->arguments()) {
        ...
        for (size_t i = 0; i < node->arguments()->size() ; i++) {
            ...
            if(!symbol->forward()) {
                if(arg->type()->name() == basic_type::TYPE_INT && symbol->elem(j - 1) == basic_type::TYPE_DOUBLE)
                    _pf.I2D();
            }
...
void m19::postfix_writer::do_alloc_node(m19::alloc_node *const node, int lvl) {
    ...
    if(node->type()->_subtype->name() == basic_type::TYPE_DOUBLE){
        _pf.INT(3);
        _pf.SHTL();
    }else if(node->type()->_subtype->name() == basic_type::TYPE_INT || node->type()->_subtype->name() == basic_type::TYPE_STRING){
        _pf.INT(2);
        _pf.SHTL();
    }
    _pf.ALLOC();
    ...
    if (_inFunctionBody) {
        if (node->init()) {
            ...
            if (node->type()->name() == basic_type::TYPE_INT || node->type()->name() == basic_type::TYPE_STRING || node->type()->name() == basic_type::TYPE_POINTER) {
                ...
                if (node->init()->type()->name() == basic_type::TYPE_INT)
                    _pf.I2D();
                _pf.LOCAL(symbol->offset());
                _pf.STDOUBLE();
