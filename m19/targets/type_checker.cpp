...
void m19::type_checker::processBinaryExpression(cdk::binary_expression_node * const node, int lvl) {
  ...
  if (node->left()->type()->name() == basic_type::TYPE_DOUBLE && node->right()->type()->name() == basic_type::TYPE_DOUBLE)
    ...
    basic_type *t = new basic_type(4, basic_type::TYPE_POINTER);
    t->_subtype = node->left()->type()->_subtype;
    node->type(t);
  }
  else if (node->left()->type()->name() == basic_type::TYPE_INT && node->right()->type()->name() == basic_type::TYPE_POINTER){
    basic_type *t = new basic_type(4, basic_type::TYPE_POINTER);
    t->_subtype = node->right()->type()->_subtype;
    node->type(t);
  }
...
void m19::type_checker::do_variable_node(cdk::variable_node * const node, int lvl) {
    ...
    if (symbol != nullptr) {
        ...
        if(symbol->type()->name() == basic_type::TYPE_POINTER)
            type->_subtype = symbol->type()->_subtype;
        node->type(type);
    } else {
        throw id;
    }
        ...
        if(node->rvalue()->type()->name() == basic_type::TYPE_POINTER){
            ...
            if(node->rvalue()->type()->_subtype->name() == basic_type::TYPE_UNSPEC){
                if(node->lvalue()->type()->_subtype->name() == basic_type::TYPE_INT)
                    node->rvalue()->type()->_subtype = new basic_type(4, basic_type::TYPE_INT);
                if(node->lvalue()->type()->_subtype->name() == basic_type::TYPE_DOUBLE)
                    node->rvalue()->type()->_subtype = new basic_type(8, basic_type::TYPE_DOUBLE);
                if(node->lvalue()->type()->_subtype->name() == basic_type::TYPE_STRING)
                    node->rvalue()->type()->_subtype = new basic_type(4, basic_type::TYPE_STRING);
            }
        }
        else if(node->rvalue()->type()->name() == basic_type::TYPE_INT) {
            //confnirmar se e zero
...
void m19::type_checker::do_for_node(m19::for_node * const node, int lvl) {    
    ...
    if(node->block() == nullptr) throw std::string("expected block on for instruction");
}
...
void m19::type_checker::do_block_node(m19::block_node *const node, int lvl) {
    if(node->declaration()){
        for(size_t i = node->declaration()->size(); i>0; i--)
            node->declaration()->node(i-1)->accept(this, lvl + 2);
    }
...
void m19::type_checker::do_alloc_node(m19::alloc_node *const node, int lvl) {    
    ...
    if (node->size()->type()->name() != basic_type::TYPE_INT) throw std::string("integer expected in size");
...
void m19::type_checker::do_address_node(m19::address_node *const node, int lvl) {
    ...
    if (node->lvalue()->type()->name() == basic_type::TYPE_DOUBLE || node->lvalue()->type()->name() == basic_type::TYPE_INT || node->lvalue()->type()->name() == basic_type::TYPE_STRING){
        basic_type *t = new basic_type(4, basic_type::TYPE_POINTER);
        t->_subtype = node->lvalue()->type();
        node->type(t);
    }
...
void m19::type_checker::do_variable_declaration_node(m19::variable_declaration_node *const node, int lvl) {
    ...
    if (node->init() != nullptr) {
        ...
        if (node->type()->name() == basic_type::TYPE_INT){
            ...
            if (node->init()->type()->name() != basic_type::TYPE_POINTER && node->init()->type()->name() != basic_type::TYPE_INT) throw std::string("wrong type, pointer expected"); 
            /*ou diferente de zero*/
        } else {
            throw std::string("unknown type");
        }
        if(node->type()->name() == basic_type::TYPE_POINTER){
            if(node->init()->type()->name() == basic_type::TYPE_POINTER && node->init()->type()->_subtype->name() == basic_type::TYPE_UNSPEC){
                if(node->type()->_subtype->name() == basic_type::TYPE_INT)
                    node->init()->type()->_subtype =  new basic_type(4, basic_type::TYPE_INT);
                if(node->type()->_subtype->name() == basic_type::TYPE_DOUBLE)
                    node->init()->type()->_subtype =  new basic_type(4, basic_type::TYPE_DOUBLE);
                if(node->type()->_subtype->name() == basic_type::TYPE_STRING)
                    node->init()->type()->_subtype =  new basic_type(4, basic_type::TYPE_STRING);
            }
        }
    }
    ...
    basic_type *t = new basic_type(node->type()->size(), node->type()->name());
    if(node->type()->name() == basic_type::TYPE_POINTER)
        t->_subtype = node->type()->_subtype;
    
    std::shared_ptr<m19::symbol> symbol = std::make_shared<m19::symbol> (t, id, node->qualifier(), (bool)node->init(), false, false);
...
void m19::type_checker::do_function_declaration_node(m19::function_declaration_node *const node, int lvl) {
    ...
    std::shared_ptr<m19::symbol> function = std::make_shared<m19::symbol> (t, id, node->qualifier(), false, true, true);
    ...
    if (previous){
        if (node->arguments()->size() != previous->args().size())
            ...
            for (size_t i = 0; i < node->arguments()->size(); i++) {
                m19::variable_declaration_node *v = dynamic_cast<m19::variable_declaration_node *> (node->arguments()->node(i));
                
                if ( (v->type()->name() == basic_type::TYPE_INT && (function->elem(i) != basic_type::TYPE_INT || function->elem(i) != basic_type::TYPE_DOUBLE) ) || (v->type()->name() == basic_type::TYPE_DOUBLE && function->elem(i) != basic_type::TYPE_DOUBLE) || (v->type()->name() == basic_type::TYPE_STRING && function->elem(i) != basic_type::TYPE_STRING))
...
void m19::type_checker::do_function_definition_node(m19::function_definition_node *const node, int lvl) {
    ...
    if(node->literal()){
        ...
        if ((node->literal()->type()->name() == basic_type::TYPE_INT && (node->type()->name() != basic_type::TYPE_INT && node->type()->name() != basic_type::TYPE_DOUBLE)) || (node->literal()->type()->name() == basic_type::TYPE_DOUBLE && node->type()->name() != basic_type::TYPE_DOUBLE) || (node->literal()->type()->name() == basic_type::TYPE_STRING && node->type()->name() != basic_type::TYPE_STRING))
            throw std::string ("incompatible types");
        
        std::string literalId = "@" + id;
        
        basic_type *tlit =  new basic_type(node->type()->size(), node->type()->name());
        if(tlit->name() == basic_type::TYPE_POINTER)
            tlit->_subtype = node->type()->_subtype;

        std::shared_ptr<m19::symbol> literal = std::make_shared<m19::symbol> (tlit, literalId, 0, false,false);
        ...
        std::shared_ptr<m19::symbol> literal = std::make_shared<m19::symbol> (t, literalId, 0, false,false);
        _symtab.insert(literalId, literal);
        _parent->set_new_symbol(literal);
    }

    std::shared_ptr<m19::symbol> function = std::make_shared<m19::symbol> (t, id, node->qualifier(), hasLiteral, true);
    function->offset(-t->size());
    
    if(node->arguments()){
        for (size_t i = 0; i < node->arguments()->size(); i++) {
            m19::variable_declaration_node *v = dynamic_cast<m19::variable_declaration_node *> (node->arguments()->node(i));
            //function->args(new basic_type(v->type()->size(), v->type()->name()));
            function->args(v->type()->name());
        }
    }
...
void m19::type_checker::do_function_call_node(m19::function_call_node *const node, int lvl) {
    ...
    if (symbol == nullptr) throw std::string("symbol function '" + id + "' undeclared");
    ...
    if(t->name() == basic_type::TYPE_POINTER)
        t->_subtype = symbol->type()->_subtype; 
    node->type(t);
    ...
    if(node->arguments() && !symbol->forward()){ 
        if (node->arguments()->size() != symbol->args().size())
            ...
            size_t j = node->arguments()->size();
            for (size_t i = 0; i < node->arguments()->size() ; i++) {
                cdk::expression_node *v = dynamic_cast<cdk::expression_node *>(node->arguments()->node(i));
                v->accept(this, lvl);

                if ((v->type()->name() == basic_type::TYPE_INT && (symbol->elem(j - 1) != basic_type::TYPE_INT && symbol->elem(j - 1) != basic_type::TYPE_DOUBLE)) || (v->type()->name() == basic_type::TYPE_DOUBLE && symbol->elem(j - 1) != basic_type::TYPE_DOUBLE) || (v->type()->name() == basic_type::TYPE_STRING && symbol->elem(j - 1) != basic_type::TYPE_STRING))
                    throw std::string("wrong type of arguments");
                j--;
